...
func (ap *Parser) Parse(feed io.Reader) (*Feed, error) {
	p := xpp.NewXMLPullParser(feed, false, util.NewReaderLabel)
	ap.base = &util.XMLBase{URIAttrs: atomURIAttrs}
...
func (ap *Parser) parseRoot(p *xpp.XMLPullParser) (*Feed, error) {
	...
	for {
		...
		if tok == xpp.StartTag {
			...
			if util.IsExtension(p) {
				e, err := util.ParseExtension(extensions, p)
				if err != nil {
					return nil, err
				}
				extensions = e
				...
				atom.Updated = result
				date, err := util.ParseDate(result)
				if err == nil {
					utcDate := date.UTC()
...
func (ap *Parser) parseAtomText(p *xpp.XMLPullParser) (string, error) {
	...
	if strings.Contains(result, "<![CDATA[") {
		...
		if lowerType == "text" ||
			...
			(lowerType == "" && lowerMode == "") {
			result, err = util.DecodeEntities(result)
